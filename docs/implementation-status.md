...
| 4. Prove exclusive maintenance and job restart | **Not started; one attempt failed and was reverted.** The two maintenance tests still time out after 60 s each. The Stage 4a attempt finished chunking twice with different tokenizer ids in the **shared** seeding helper; that throws during seeding (`seeding nightfall failed against collections`) and regressed the marked-generation test, so the controller reverted it — the shared helper must keep a current tokenizer id, because Stage 2's tightened `validate` depends on the seed being coherent. **Next attempt must:** set the stale tokenizer **only** in the two maintenance tests' own seed, and express staleness by writing the chunking row directly rather than calling `finishChunking` twice. |
