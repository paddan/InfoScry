...
| 20 | **In progress.** Part A (`863095f`) — the provider-neutral contract, retry policy and SSE scanner, 16 tests. Part B (`1784ba5`) — the OpenAI-compatible adapter with seven tests including the Review Focus 5 leak test, saved by the controller after three runs died at the commit step. **Part C remaining:** the Anthropic adapter, the capability wiring that makes `llm test` real, the `llm test` existence check, the `model_calls.endpoint` sanitisation, and a retry-after-delta test. |
| 21–28 | Not started. |
